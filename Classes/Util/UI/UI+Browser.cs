    ...
    public static partial class UI {
        ...
        public class Browser<Definition, Item> : Browser {
            ...
            public int SearchLimit {
                get => IsDetailBrowser ? Main.settings.browserDetailSearchLimit : Main.settings.browserSearchLimit;
                set {
                    var oldValue = SearchLimit;
                    if (IsDetailBrowser)
                        Main.settings.browserDetailSearchLimit = value;
                    else
                        Main.settings.browserSearchLimit = value;
                    if (value != oldValue) Main.settings.Save(Main.modEntry);
                }
            }
                ...
                if (search || SearchLimit < _matchCount) {
                    if (search) {
                        using (HorizontalScope()) {
                            ...
                            if (isCollating) {
                                Label("Collating...".cyan().bold(), AutoWidth());
                                25.space();
                            } else if (_doCopyToEnd) {
                                Label("Copying...".cyan().bold(), AutoWidth());
                                25.space();
                            }
                        }
                    } else {
                    ...
                    using (HorizontalScope()) {
                        ...
                        if (_matchCount > 0 || _searchText.Length > 0) {
                            string? matchesText = "Matches: ".Green().Bold() + $"{_matchCount}".Orange().Bold();
